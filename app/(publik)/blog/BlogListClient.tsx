...
import { Terminal, ArrowRight, ChevronLeft, ChevronRight, Search, X } from "lucide-react";
  ...
  return (
    <Link href={`/blog/${artikel.slug}`} className="group relative grid grid-cols-1 lg:grid-cols-12 gap-0 overflow-hidden rounded-xl border border-white/5 bg-secondary-bg hover:border-primary/40 transition-all duration-500">
      ...
      <div className="lg:col-span-5 p-8 lg:p-12 flex flex-col justify-center">
        ...
        <h2 className="text-2xl md:text-3xl font-bold mb-4 leading-tight text-foreground group-hover:text-primary transition-colors">
          {artikel.judul}
        </h2>
  ...
  return (
    <Link href={`/blog/${artikel.slug}`} className="group flex flex-col bg-secondary-bg rounded-xl border border-white/5 overflow-hidden hover:border-primary/40 transition-all duration-300">
      ...
      <div className="p-6 flex flex-col flex-1">
        ...
        <h3 className="text-lg font-bold mb-3 leading-snug text-foreground group-hover:text-primary transition-colors">
          {artikel.judul}
        </h3>
        ...
        <div className="mt-auto pt-6 border-t border-white/5 flex items-center justify-between">
          <span className="text-xs font-mono text-muted">READ MORE</span>
...
export default function BlogListClient({ artikelList }: { artikelList: Artikel[] }) {
  const [currentPage, setCurrentPage] = useState(1);
  const [searchQuery, setSearchQuery] = useState("");

  // Filter articles by search query
  const filteredList = useMemo(() => {
    if (!searchQuery.trim()) return artikelList;
    const q = searchQuery.toLowerCase();
    return artikelList.filter(
      (a) =>
        a.judul.toLowerCase().includes(q) ||
        (a.konten && a.konten.toLowerCase().includes(q))
    );
  }, [artikelList, searchQuery]);
  ...
  return (
    <div className="py-12">
      ...
      <div className="flex flex-col gap-2 mb-10">
        ...
        <h1 className="text-4xl md:text-5xl font-black tracking-tight text-foreground">Artikel Terbaru</h1>
      ...
      <div className="mb-14">
        <div className="relative max-w-xl">
          ...
          <input
            ...
            className="w-full pl-12 pr-10 py-3 rounded-xl border border-white/10 bg-secondary-bg text-foreground placeholder-muted text-sm focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary/50 transition-all"
          />
          {searchQuery && (
            <button
        ...
        {searchQuery && (
          <p className="text-sm text-muted mt-3 font-mono">
            {filteredList.length} artikel ditemukan untuk &quot;{searchQuery}&quot;
          </p>
        )}
      </div>
            ...
            <button
              ...
              className={`size-10 flex items-center justify-center rounded text-sm font-mono transition-all ${
                currentPage === i + 1
                  ...
                  : "border border-white/10 text-muted hover:text-foreground hover:border-primary"
              }`}
            >
              {i + 1}
            </button>
